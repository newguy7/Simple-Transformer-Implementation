...
class AutoRegressiveWrapper(nn.Module):
    ...
    def generate(self, start_tokens, seq_len, eos_token = None, temperature = 1, filter_thres = 0.9):
        ...
        if num_dims == 1:
            start_tokens = start_tokens[None, :] # 1024 --> (1,1024); [None, :] is an alias for np.newaxis,it will add one more direction
            ...
            for _ in range(seq_len): # seq_len = e.g. 1024
                ...
                out = torch.cat((prev_out, predicted_char_token), dim=-1) # (1x1025)
    ...
    def forward(self,x):
        ...
        x0 = x[:, 1:] # expected output in training is shifted one to the right
        out = self.model(xi)
        logits_reorg = out.view(-1, out.size(-1))
        targets_reorg = x0.reshape(-1)
